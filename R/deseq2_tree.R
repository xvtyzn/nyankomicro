...
deseq2_tree <- function(physeq, deseq2_results, level = c("Kingdom", "Phylum",
    ...
    gh <- tt %>% ggplot() + geom_tile(aes(x = Sample, y = ASV, fill = sample_annotation,
    ...
    fc_gg <- sigtab %>% ggplot(aes(y = ASV, x = log2FoldChange, color = level)) +
    ...
    p_gg <- sigtab %>% mutate(log2padj = -log2(padj)) %>% ggplot(aes(y = ASV,
        x = log2padj, fill = level)) + geom_bar(stat = "identity") + ylab(NULL) +
